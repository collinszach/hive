...
function MerchantsPageInner() {
  ...
  useEffect(() => {
    setLoading(true);
    api.merchants.list({ days, limit: 30 }).then((list) => {
      setMerchants(list);
      // Auto-select from ?m= query param (deep link from dashboard)
      const mParam = searchParams.get("m");
      if (mParam && !selected) {
        setDetailLoading(true);
        api.merchants.history(mParam)
          .then((d) => { setSelected(mParam); setDetail(d); })
          .catch(() => toast.error("Failed to load merchant details"))
  ...
  async function handleBulkRecategorize() {
    ...
    try {
      ...
      const d = await api.merchants.history(selected);
      setDetail(d);
      toast.success(`Recategorizing all ${selected} transactions…`);
    } catch {
      setBulkStatus("error");
          ...
          <div className="divide-y divide-white/[0.04]">
            {detail.transactions.map((t) => (
              <div key={t.id} className="flex items-center justify-between px-5 py-3 gap-4">
                ...
                <button
                  onClick={() => router.push(`/transactions?open_tx=${t.id}&search_all=true${selected ? `&search=${encodeURIComponent(selected)}` : ""}`)}
                  className="text-ink-ghost hover:text-honey transition-colors shrink-0 p-1"
