...
export default function AccountPage() {
  ...
  async function startTotpSetup() {
    ...
    if (!r.ok) return flash("err", "Failed to start MFA setup");
  ...
  async function verifyTotp(e: FormEvent) {
    ...
    if (!r.ok) {
      ...
      return flash("err", d.detail ?? "Invalid code");
    }
    flash("ok", "MFA enabled — your account is now protected with two-factor authentication");
    setTotpSetup(null);
    setTotpVerifyCode("");
    setShowQr(false);
  ...
  async function changePassword(e: FormEvent) {
    ...
    if (!r.ok) {
      const d = await r.json();
      return flash("err", d.detail ?? "Failed to change password");
    }
    ...
    <div className="space-y-5 max-w-2xl animate-fade-in">
      ...
      {msg && (
        <div className={`flex items-center gap-2 rounded-lg px-4 py-3 border text-sm ${
          msg.type === "ok"
            ? "bg-semantic-income/[0.08] border-semantic-income/20 text-semantic-income"
            : "bg-semantic-expense/[0.08] border-semantic-expense/20 text-semantic-expense"
        }`}>
          {msg.type === "ok"
