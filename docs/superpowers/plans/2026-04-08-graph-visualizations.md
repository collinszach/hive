# Graph Visualizations Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace the Sankey income flow and dashboard account cards with two SVG graph visualizations: an n8n-style node flow and an Obsidian-style layered account graph with drill-down.

**Architecture:** Pure SVG components — no new npm dependencies. `NodeFlow.tsx` replaces `SankeyFlow.tsx` with proportional n8n-style nodes and bezier wires. `AccountGraph.tsx` is a new self-contained component with a three-state machine (graph → account detail → transactions) managing its own data fetching.

**Tech Stack:** React + SVG, Next.js 14 App Router, existing `api.ts` client, Tailwind for non-SVG UI
...
| Action | File | Change |
|--------|------|--------|
| Modify | `backend/app/api/reports.py` | Add `account_id` filter to spending-by-category |
| Modify | `frontend/src/lib/api.ts` | Add `account_id` param to `reports.spendingByCategory` |
| Create | `frontend/src/app/cash-flow/_components/NodeFlow.tsx` | n8n node flow component |
| Modify | `frontend/src/app/cash-flow/page.tsx` | Swap `<SankeyFlow>` → `<NodeFlow>` |
| Delete | `frontend/src/app/cash-flow/_components/SankeyFlow.tsx` | Replaced by NodeFlow |
| Create | `frontend/src/components/AccountGraph.tsx` | Layered account graph with drill-down |
| Modify | `frontend/src/app/page.tsx` | Swap account cards section → `<AccountGraph>` |

---

## Task 1: Backend — `account_id` filter on spending-by-category
...
```bash
cat backend/app/api/reports.py
```

The file uses raw SQL via `text()`. The `spending_by_category` endpoint currently takes `start_date` and `end_date`. We need to add an optional `account_id` query param and add it to the WHERE clause when present.

- [ ] **Add the `account_id` parameter and filter**

Replace the `spending_by_category` function signature and query. The full new function:
...
```bash
cd /home/zach/hive && docker compose exec backend python -c "from app.api.reports import router; print('ok')"
```
...
git commit -m "feat(reports): add optional account_id filter to spending-by-category"
```

---

## Task 2: Frontend API client — `account_id` on `spendingByCategory`

**Files:**
- Modify: `frontend/src/lib/api.ts` (line ~669)

- [ ] **Update `reports.spendingByCategory`**
...
git commit -m "feat(api): add account_id param to spendingByCategory"
```

---

## Task 3: Create `NodeFlow.tsx`

**Files:**
- Create: `frontend/src/app/cash-flow/_components/NodeFlow.tsx`

The component accepts `FlowData` (same shape as `SankeyFlow`). It renders an SVG with one source node (Income, fixed size) and destination nodes (categories + savings) whose height scales proportionally to their amount. Bezier S-curve wires connect them; wire stroke-width also scales with amount.
...
```bash
git add frontend/src/app/cash-flow/_components/NodeFlow.tsx
git commit -m "feat(cash-flow): add NodeFlow n8n-style income node graph"
```

---

## Task 4: Swap SankeyFlow → NodeFlow in cash-flow page

**Files:**
- Modify: `frontend/src/app/cash-flow/page.tsx`
- Delete: `frontend/src/app/cash-flow/_components/SankeyFlow.tsx`
...
- [ ] **Swap the JSX element**

Find (around line 213–223):
```tsx
...
```bash
rm frontend/src/app/cash-flow/_components/SankeyFlow.tsx
```

- [ ] **Verify the build**

```bash
cd /home/zach/hive && docker compose build frontend 2>&1 | tail -20
```

Expected: build completes without TypeScript errors referencing SankeyFlow or NodeFlow.

- [ ] **Commit**

```bash
git add frontend/src/app/cash-flow/page.tsx
git rm frontend/src/app/cash-flow/_components/SankeyFlow.tsx
git commit -m "feat(cash-flow): swap SankeyFlow for NodeFlow, delete old component"
```

---

## Task 5: Create `AccountGraph.tsx`

**Files:**
- Create: `frontend/src/components/AccountGraph.tsx`

Self-contained component. Manages its own data fetching. Three-state machine:
- **State 1** (`kind: "graph"`): Full SVG with all accounts in layered layout, net worth at top.
- **State 2** (`kind: "account"`): Clicked account node centered; category spend nodes beside it; month nav.
- **State 3** (`kind: "transactions"`): Graph shrinks; transaction list panel below.
...
```bash
git add frontend/src/components/AccountGraph.tsx
git commit -m "feat(dashboard): add AccountGraph layered Obsidian-style account graph with drill-down"
```

---

## Task 6: Swap account cards for `AccountGraph` in the dashboard

**Files:**
- Modify: `frontend/src/app/page.tsx`

The dashboard currently renders `AccountCard` components in two grids (bank accounts and credit cards). We replace that entire section with `<AccountGraph />`.
...
- [ ] **Find and replace the accounts section**

Search for the JSX block that renders `AccountCard` components. It looks like this (around line 770–810):
```tsx
{/* Accounts section — bank + credit cards in two grids */}
...
...
Replace the entire card grid section (both grids and their wrapping containers) with:
```tsx
<AccountGraph />
```

The `AccountGraph` component self-fetches all data it needs, so remove the `accts` state usage from the surrounding JSX (but leave the state variable — it may still be used elsewhere in the file for the empty state check).

- [ ] **Check that the dashboard still compiles**

```bash
cd /home/zach/hive && docker compose build frontend 2>&1 | tail -30
```
...
git commit -m "feat(dashboard): replace account cards grid with AccountGraph"
```
...
```bash
cd /home/zach/hive && docker compose build backend frontend
```

- [ ] **Restart with native-db override**

```bash
docker compose -f docker-compose.yml -f docker-compose.native-db.yml up -d backend frontend
```
...
Expected: backend and frontend both show `(healthy)` or `Up`.

- [ ] **Smoke test the cash-flow page**

Navigate to `/cash-flow` — the Income Flow section should render node cards connected by bezier wires instead of ribbon bands.

- [ ] **Smoke test the dashboard graph**

Navigate to `/` — the account section should show a layered SVG graph. Click any account node to enter the category drill-down. Click a category to see the transaction panel.
