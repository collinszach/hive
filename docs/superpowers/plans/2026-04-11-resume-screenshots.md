# Resume Screenshots Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Generate 12 portfolio-quality PNG screenshots of the Hive finance platform with realistic fake data (persona: "Alex Rivera"), using Playwright to intercept all API calls at the network level.

**Architecture:** A single Node.js script (`scripts/take-screenshots.js`) uses Playwright's `page.route()` to intercept every `/api/*` request before it hits the network, returning fixture data from `scripts/demo-fixtures.js`. The live Next.js frontend at `localhost:3000` renders normally and is screenshotted at 1440×900. Zero changes to app code.

**Tech Stack:** Node.js 20, Playwright (chromium), Next.js app already running at `localhost:3000`
...
| File | Action | Purpose |
|---|---|---|
| `scripts/demo-fixtures.js` | Create | All mock API responses keyed by URL path pattern |
| `scripts/take-screenshots.js` | Create | Playwright orchestration: route interception + navigate + screenshot |
| `screenshots/` | Create (dir) | Output directory for 12 PNGs |
| `package.json` (root) | Modify | Add `playwright` as devDependency |

---

## Task 1: Install Playwright and verify the app is reachable
...
Expected: downloads Chromium, ends with `✓ chromium ... installed`

- [ ] **Step 3: Create the output directory**

```bash
mkdir -p /home/zach/hive/screenshots
```

- [ ] **Step 4: Verify the frontend is running**

```bash
curl -s --max-time 3 http://localhost:3000 | head -c 50
```

Expected output includes: `<!DOCTYPE html>` — if blank, start the frontend with `cd frontend && npm run dev`
...
git commit -m "chore: add playwright dev dependency for screenshot script"
```

---

## Task 2: Write demo-fixtures.js

**Files:**
- Create: `scripts/demo-fixtures.js`

This file exports all mock API responses that the 12 app pages need.

- [ ] **Step 1: Create the scripts directory and write the file**

```bash
mkdir -p /home/zach/hive/scripts
```

Now write the file at `scripts/demo-fixtures.js`:
...
function netWorthHistory() {
  ...
  for (let i = 0; i < 90; i++) {
    ...
    const trend = i * 140; // ~$140/day net worth growth
...
const ANOMALIES = [
  {
    ...
    reason: "Transaction amount ($847) is 6.2× your average spend at this merchant ($137). Unusual time of day.",
  ...
  },
  {
    ...
    reason: "First-ever transaction at this merchant. Category 'Travel > Hotel' is rare for your history.",
...
const GOALS = [
  ...
  {
    id: "g2", name: "Japan Trip ✈️", description: "Two weeks in Tokyo & Kyoto",
...
// ─── Dispatcher ───────────────────────────────────────────────────────────────
// Given a URL pathname + search params, return the fixture to serve (or null to pass-through).
...
function getFixture(pathname, searchParams) {
  ...
  // Points — order matters: specific before generic
  ...
  // Transactions — specific before generic
  ...
  // Chat (POST — returns a scripted response)
  ...
  // Plaid / SnapTrade / Tax — not needed for screenshots
  return null;
}
...
```bash
node -e "require('./scripts/demo-fixtures.js'); console.log('OK')"
```
...
```bash
git add scripts/demo-fixtures.js
git commit -m "feat: add demo fixture data for screenshot script"
```

---

## Task 3: Write take-screenshots.js
...
Create `scripts/take-screenshots.js` with this content:
...
```bash
node --check scripts/take-screenshots.js && echo "Syntax OK"
```
...
```bash
git add scripts/take-screenshots.js
git commit -m "feat: add Playwright screenshot script with demo route interception"
```

---

## Task 4: Run the script and verify all 12 screenshots

**Files:**
- Verify: `screenshots/*.png` — all 12 files exist and are non-empty

- [ ] **Step 1: Confirm the frontend is running on port 3000**

```bash
curl -s --max-time 3 http://localhost:3000 | grep -o "<title>[^<]*" | head -1
```

Expected output: `<title>Hive — Personal Finance`

If blank: start the frontend with `cd /home/zach/hive/frontend && npm run dev &` and wait 10 seconds.
...
Expected output:
```
Hive Demo Screenshots → /home/zach/hive/screenshots

  → Dashboard (/)
    ✓ saved dashboard.png
  → Transactions (/transactions)
    ✓ saved transactions.png
  → Cash Flow (/cash-flow)
    ✓ saved cash-flow.png
  → Points & Rewards (/points)
    ✓ saved points.png
  → Card Optimizer (/optimize)
    ✓ saved optimize.png
  → Net Worth (/net-worth)
    ✓ saved net-worth.png
  → Anomalies (/anomalies)
...
Done. 12 screenshots saved to screenshots/
```

- [ ] **Step 3: Verify all 12 files exist and are non-empty**

```bash
ls -lh /home/zach/hive/screenshots/
```

Expected: 12 `.png` files, each at least 100 KB (2× retina images will be 300–800 KB each).

- [ ] **Step 4: If any screenshot shows a blank/login page, debug**

The most common issue is auth redirect. Verify the fixture is matching by adding a debug line:

```bash
node -e "
const { getFixture } = require('./scripts/demo-fixtures.js');
const { URLSearchParams } = require('url');
console.log(JSON.stringify(getFixture('/api/auth/me', new URLSearchParams()), null, 2));
"
```

Expected: prints the AUTH_ME object. If it prints `null`, the dispatcher has a bug — check the `pathname === '/api/auth/me'` line in demo-fixtures.js.

- [ ] **Step 5: Commit screenshots and close the plan**

```bash
cd /home/zach/hive
git add screenshots/
git commit -m "feat: add resume demo screenshots (12 pages, Alex Rivera persona)"
```
...
**Screenshot shows loading spinner:** Increase the `waitForTimeout` value in `screenshotPage()` from `1800` to `3000`.

**Screenshot shows login page:** The `/api/auth/me` route isn't being intercepted. Check that `installRoutes()` is called before `page.goto()` and that the pattern `**/api/**` matches.

**Playwright not found:** Run `npm install` in the project root, then `npx playwright install chromium`.

**Recharts/animated charts are empty:** The `waitForTimeout` may need to be longer. Try `2500`.
