...
# Redemption nudge thresholds (points) — mirrors CLAUDE.md spec
_REDEMPTION_THRESHOLDS: dict[str, int] = {
    "Chase UR": 60_000,
    "Amex MR": 75_000,
    "SW RR": 50_000,
    "Bilt Points": 50_000,
...
def generate_insights() -> dict:
    ...
    with SyncSessionLocal() as session:
        ...
        # ------------------------------------------------------------------ #
        # 5. REWARD THRESHOLD — points balance above redemption nudge          #
        # ------------------------------------------------------------------ #
        # Get the most recent balance per program
        recent_balances: list[PointsBalance] = session.execute(
            text("""
                SELECT DISTINCT ON (program)
                    id, program, card_slug, balance, as_of
        ...
        for row in recent_balances:
            threshold = _REDEMPTION_THRESHOLDS.get(row.program)
            if threshold is None or row.balance < threshold:
                continue
            dedup_key = f"reward_threshold:{row.program}:{today.strftime('%Y-%m')}"
            exists = session.execute(
                select(Insight).where(Insight.dedup_key == dedup_key)
            ).scalar_one_or_none()
            if not exists:
                session.add(Insight(
                    id=uuid.uuid4(),
                    insight_type="reward_threshold",
                    title=f"{row.program} above redemption threshold",
                    body=(
                        f"You have {row.balance:,} {row.program} points — "
                        f"above your {threshold:,}-point redemption threshold. "
                        f"Consider redeeming for maximum value."
                    ),
                    amount=float(row.balance),
                    linked_entity_type="points_program",
                    linked_entity_id=row.program,
                    priority="medium",
                    dedup_key=dedup_key,
                ))
                generated += 1
