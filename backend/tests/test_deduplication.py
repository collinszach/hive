...
class TestUpsertLogic:
    ...
    def test_upsert_uses_plaid_transaction_id_as_conflict_target(self):
        ...
        mock_db = MagicMock()
        # First execute call: load custom rules (return empty list)
        ...
        assert count == 1
        # execute is called twice: (1) load active custom rules, (2) pg_insert upsert
        assert mock_db.execute.call_count == 2
        # Verify the second call uses a pg INSERT statement (not a plain SELECT)
        from sqlalchemy.dialects.postgresql.dml import Insert as PgInsert
        insert_stmt = mock_db.execute.call_args_list[1][0][0]
        assert isinstance(insert_stmt, PgInsert), (
            f"Expected a PostgreSQL INSERT statement, got {type(insert_stmt)}"
        )
        # Verify conflict target includes plaid_transaction_id
        conflict = insert_stmt._post_values_clause
        target_elements = conflict.inferred_target_elements or conflict.constraint_target
        element_names = [
            (e.key if hasattr(e, "key") else str(e))
            for e in (target_elements or [])
        ]
        assert "plaid_transaction_id" in element_names, (
            f"Expected plaid_transaction_id in conflict target, got {element_names}"
        )
    ...
    def test_unknown_account_id_is_skipped(self):
        ...
        count = _upsert_transactions(mock_db, account_map, [mock_tx])
        assert count == 0
        # One execute call to load custom rules; no INSERT since all txns were skipped
