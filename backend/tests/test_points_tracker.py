...
class TestWFAutograph:
    def test_gas_3x(self):
        result = compute_points_for_transaction("wf_autograph", "Transportation", "Gas", 60.0)
        assert result is not None
        assert result.earn_rate == 3.0
        assert result.program == "WF Rewards"

    def test_ev_charging_3x(self):
        result = compute_points_for_transaction("wf_autograph", "Transportation", "EV Charging", 30.0)
        assert result is not None
        assert result.earn_rate == 3.0

    def test_transit_3x(self):
        result = compute_points_for_transaction("wf_autograph", "Transportation", "Transit", 20.0)
        assert result is not None
        assert result.earn_rate == 3.0

    def test_tolls_3x(self):
        result = compute_points_for_transaction("wf_autograph", "Transportation", "Tolls", 5.0)
        assert result is not None
        assert result.earn_rate == 3.0

    def test_entertainment_3x(self):
        """Autograph gives 3x on all Entertainment — category-only match."""
        result = compute_points_for_transaction("wf_autograph", "Entertainment", "Events", 75.0)
        assert result is not None
        assert result.earn_rate == 3.0

    def test_phone_3x(self):
        result = compute_points_for_transaction("wf_autograph", "Utilities", "Phone", 80.0)
        assert result is not None
        assert result.earn_rate == 3.0

    def test_base_1x(self):
        result = compute_points_for_transaction("wf_autograph", "Shopping", "Clothing", 100.0)
        assert result is not None
        assert result.earn_rate == 1.0
...
class TestCardOptimizer:
    def test_returns_all_six_cards(self):
